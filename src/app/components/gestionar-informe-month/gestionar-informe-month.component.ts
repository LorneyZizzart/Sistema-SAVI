...
@Component({
  ...
  templateUrl: './gestionar-informe-month.component.html',
  styleUrls: ['./gestionar-informe-month.component.css']
})
export class GestionarInformeMonthComponent implements OnInit {
   ...
   //Registro de month
   informesRegistrosMonth:RegistroHora[];
   //FOOTER
   numStudent:number = 0;
   numHour:any = '00:00';
   totalSaldo:any = 0;
  ...
  this.getInformeRegisterMonth(this.IdDepartamento);
  setTimeout(() => {
  ...
  starMonth(){
    var fecha=new Date();
    var haceUnMes=new Date(fecha.getTime() - (24*60*60*1000)*30);
    var dd = haceUnMes.getDate();
    var mm = haceUnMes.getMonth() + 1;
    var yyy = haceUnMes.getFullYear();
    var dia = dd.toString();
    var mes = mm.toString();
    if (dd < 10) { dia = '0' + dd; }
    if (mm < 10) { mes = '0' + mm; }
  ...
  totalDatos(){
    ...
    var numMoney;
    for(let registro of this.informesRegistrosMonth){
      this.numStudent++;
...
informacionEstudiante(idEstudiante){
  ...
  registrarAprovacion(idRegistro:string, aprobado:string){
